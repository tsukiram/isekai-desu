...
const NavigationBar = ({ isLoggedIn, setIsLoggedIn }) => {
    ...
    const handleLogout = async () => {
        try {
            const token = localStorage.getItem('token');
            if (token) {
                const response = await axios.post('http://34.41.183.238:8000/api/users/logout/', null, {
                    headers: { Authorization: `Token ${token}` }
                });
                        ...
                        {isLoggedIn ? (
                            <>
                                ...
                                <div className="profil-container" onClick={togglePopover}>
                                    <Image src={userProfile?.image || profil} alt="Profil" className="profil" style={{ width: "3rem", height: "3rem", borderRadius: "50%" }}/>
                                    {showPopover && (
