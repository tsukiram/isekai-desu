...
const DetailNovelContent = () => {
    ...
    const fetchPost = async () => {
        try {
            const response = await axios.get(`http://34.41.183.238:8000/api/posts/${id}/`);
    ...
    const fetchComments = async () => {
        try {
            const response = await axios.get(`http://34.41.183.238:8000/api/posts/${id}/comments/`);
    ...
    const fetchUserProfile = async () => {
        try {
            ...
            const response = await axios.get(`http://34.41.183.238:8000/api/users/profile/`, {
    ...
    const handleToggleFavorite = async () => {
        try {
            ...
            const response = await axios.post('http://34.41.183.238:8000/api/favorite/toggle/', 
            { post_id: id }, 
            { headers: { Authorization: `Token ${token}` } });
    ...
    const handleNewCommentSubmit = async (e) => {
        ...
        if (newComment.trim()) {
            try {
                const token = localStorage.getItem('token');
                const response = await axios.post(`http://34.41.183.238:8000/api/comments/create/`, {
                    post: id,
                    content: newComment,
                }, {
                    headers: { Authorization: `Token ${token}` }
    ...
    const confirmDelete = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://34.41.183.238:8000/api/comments/${commentToDelete.id}/delete/`, {
                headers: { Authorization: `Token ${token}` }
            });
    ...
    const handleDeletePost = async () => {
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://34.41.183.238:8000/api/posts/${id}/delete/`, {
                headers: { Authorization: `Token ${token}` }
            });
            navigate('/');
        } catch (error) {
